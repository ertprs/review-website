...
const createHeaderData = (data) => {
    ...
    willCome = data.hasOwnProperty('domain_data') || data.hasOwnProperty('reviews')
        || data.hasOwnProperty('general_analysis') || isScheduled
    const parsedData = {
        ...parsedData,
...
const createAnalysisData = (data) => {
    ...
    let doesKeyExist = data.hasOwnProperty('whois') || data.hasOwnProperty('ssl')
        || data.hasOwnProperty('etherscam') || data.hasOwnProperty('phishtank') ||
        data.hasOwnProperty('wot') || data.hasOwnProperty('deface') || isScheduled
...
const createTrafficReports = (data) => {
    ...
    if (isScheduled || data.hasOwnProperty('traffic')) {
        if ((_get(data, 'traffic.payload.timeline', []) !== null) && (Array.isArray(_get(data, 'traffic.payload.timeline', [])))) {
    ...
    if (Array.isArray(_get(data, "traffic.payload.timeline", []))) {
        if (_get(data, "traffic.payload.timeline", []).length > 0) {
            ...
            parsedData = {
                ...parsedData,
                daily_unique_visitors: _get(timeline[0], "visits.daily_unique_visitors", 0),
                monthly_unique_visitors: _get(timeline[0], "visits.monthly_unique_visitors", 0),
...
const createDomainReviews = (data) => {
    ...
    if (isScheduled || data.hasOwnProperty('reviews')) {
        if ((_get(data, 'reviews.domain.reviews', []) !== null) && (Array.isArray(_get(data, 'reviews.domain.reviews', [])))) {
